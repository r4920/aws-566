...
const addEvent = async (req, res) => {
  try {
    ...
    let createdEvent = await dbService.createDocument(Event,dataToCreate);
...
const findAllEvent = async (req,res) => {
  try {
    ...
    let foundEvents = await dbService.getAllDocuments( Event,query,options);
...
const getEventCount = async (req,res) => {
  try {
    ...
    let countedEvent = await dbService.countDocument(Event,where);
...
const softDeleteManyEvent = async (req,res) => {
  try {
    ...
    let updatedEvent = await dbService.bulkUpdate(Event,query, updateBody);
    if (!updatedEvent) {
      return res.recordNotFound();
    }
...
const bulkInsertEvent = async (req,res)=>{
  try {
    ...
    let createdEvents = await dbService.bulkInsert(Event,dataToCreate);
    return res.success({ data :createdEvents });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
    }
...
const bulkUpdateEvent = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkUpdate(Event,filter,dataToUpdate);
    if (!result){
      return res.recordNotFound();
    }
...
const softDeleteEvent = async (req,res) => {
  try {
    ...
    let updatedEvent = await dbService.findOneAndUpdateDocument(Event, query, updateBody,{ new:true });
    if (!updatedEvent){
      return res.recordNotFound();
    }
    return res.success({ data:updatedEvent });
...
const partialUpdateEvent = async (req,res) => {
  try {
    ...
    let updatedEvent = await dbService.findOneAndUpdateDocument(Event, query, dataToUpdate,{ new:true });
    if (!updatedEvent) {
      return res.recordNotFound();
    }
    return res.success({ data:updatedEvent });
...
const getEvent = async (req,res) => {
  try {
    ...
    let options = {};
    let foundEvent = await dbService.getSingleDocument(Event,query, options);
    if (!foundEvent){
      return res.recordNotFound();
    }
...
const deleteEvent = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedEvent = await dbService.findOneAndDeleteDocument(Event, query);
    if (!deletedEvent){
      return res.recordNotFound();
    }
    return res.success({ data :deletedEvent });
