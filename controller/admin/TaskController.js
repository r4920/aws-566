...
const addTask = async (req, res) => {
  try {
    ...
    let createdTask = await dbService.createDocument(Task,dataToCreate);
...
const findAllTask = async (req,res) => {
  try {
    ...
    let foundTasks = await dbService.getAllDocuments( Task,query,options);
...
const getTaskCount = async (req,res) => {
  try {
    ...
    let countedTask = await dbService.countDocument(Task,where);
...
const softDeleteManyTask = async (req,res) => {
  try {
    ...
    let updatedTask = await dbService.bulkUpdate(Task,query, updateBody);
    if (!updatedTask) {
      return res.recordNotFound();
    }
...
const bulkInsertTask = async (req,res)=>{
  try {
    ...
    let createdTasks = await dbService.bulkInsert(Task,dataToCreate);
    return res.success({ data :createdTasks });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
    }
...
const bulkUpdateTask = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkUpdate(Task,filter,dataToUpdate);
    if (!result){
      return res.recordNotFound();
    }
...
const softDeleteTask = async (req,res) => {
  try {
    ...
    let updatedTask = await dbService.findOneAndUpdateDocument(Task, query, updateBody,{ new:true });
    if (!updatedTask){
      return res.recordNotFound();
    }
    return res.success({ data:updatedTask });
...
const partialUpdateTask = async (req,res) => {
  try {
    ...
    let updatedTask = await dbService.findOneAndUpdateDocument(Task, query, dataToUpdate,{ new:true });
    if (!updatedTask) {
      return res.recordNotFound();
    }
    return res.success({ data:updatedTask });
...
const getTask = async (req,res) => {
  try {
    ...
    let options = {};
    let foundTask = await dbService.getSingleDocument(Task,query, options);
    if (!foundTask){
      return res.recordNotFound();
    }
...
const deleteTask = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedTask = await dbService.findOneAndDeleteDocument(Task, query);
    if (!deletedTask){
      return res.recordNotFound();
    }
    return res.success({ data :deletedTask });
