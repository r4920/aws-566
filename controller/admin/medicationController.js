...
const addMedication = async (req, res) => {
  try {
    ...
    let createdMedication = await dbService.createDocument(Medication,dataToCreate);
...
const findAllMedication = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(Medication, query);
      return res.success({ data: { totalRecords } });
    }
    if (req.body && typeof req.body.options === 'object' && req.body.options !== null) {
      options = { ...req.body.options };
    }
    let foundMedications = await dbService.getAllDocuments( Medication,query,options);
...
const getMedicationCount = async (req,res) => {
  try {
    ...
    let countedMedication = await dbService.countDocument(Medication,where);
...
const softDeleteManyMedication = async (req,res) => {
  try {
    ...
    let updatedMedication = await dbService.bulkUpdate(Medication,query, updateBody);
...
const bulkInsertMedication = async (req,res)=>{
  try {
    ...
    let createdMedications = await dbService.bulkInsert(Medication,dataToCreate);
...
const bulkUpdateMedication = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkUpdate(Medication,filter,dataToUpdate);
    if (!result){
      return res.recordNotFound();
    }
...
const softDeleteMedication = async (req,res) => {
  try {
    ...
    let updatedMedication = await dbService.findOneAndUpdateDocument(Medication, query, updateBody,{ new:true });
    if (!updatedMedication){
      return res.recordNotFound();
    }
    return res.success({ data:updatedMedication });
...
const partialUpdateMedication = async (req,res) => {
  try {
    ...
    let updatedMedication = await dbService.findOneAndUpdateDocument(Medication, query, dataToUpdate,{ new:true });
    if (!updatedMedication) {
      return res.recordNotFound();
    }
    return res.success({ data:updatedMedication });
...
const getMedication = async (req,res) => {
  try {
    ...
    let options = {};
    let foundMedication = await dbService.getSingleDocument(Medication,query, options);
    if (!foundMedication){
      return res.recordNotFound();
    }
...
const deleteMedication = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedMedication = await dbService.findOneAndDeleteDocument(Medication, query);
    if (!deletedMedication){
      return res.recordNotFound();
    }
    return res.success({ data :deletedMedication });
