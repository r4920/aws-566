...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'i2pDbkXFbP3KAat',
      'username':'Rosario.Jacobi26',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Rosario.Jacobi26' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'CC507HzsiDeRxzY',
      'username':'Augusta21',
      'isDeleted':false,
      'isActive':true,
    ...
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Augusta21' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
